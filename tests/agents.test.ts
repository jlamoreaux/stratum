...
import { createAgent, getAgent, listAgents, deleteAgent } from '../src/storage/agents';
...
describe('POST /api/agents', () => {
  ...
  it('creates agent with user auth and returns 201', async () => {
    ...
    expect(createAgent).toHaveBeenCalledWith(env.DB, 'usr_owner', 'my-agent', undefined, undefined, undefined);
  });
  ...
  it('creates agent with optional model field', async () => {
    ...
    const res = await app.fetch(
      request('POST', '/api/agents', { name: 'my-agent', model: 'claude-3-5-sonnet' }, userAuthHeader),
      env,
    );
    expect(res.status).toBe(201);
    expect(createAgent).toHaveBeenCalledWith(env.DB, 'usr_owner', 'my-agent', 'claude-3-5-sonnet', undefined, undefined);
  });
  ...
  it('returns 400 when name is not a string', async () => {
    const res = await app.fetch(
      request('POST', '/api/agents', { name: 42 }, userAuthHeader),
      env,
    );
    expect(res.status).toBe(400);
  });
