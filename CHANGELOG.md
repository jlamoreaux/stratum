...
- **The legacy `POST /auth/email/send` no longer starts a signup under the gate.**
  ...
  "invalid invite code" at the end of it. It now refuses up front and points at
  `/auth/signup`, where a code can be given. Login links for existing accounts
  are unaffected, as is the endpoint with the gate off.
