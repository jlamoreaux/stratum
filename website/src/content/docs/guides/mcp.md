...
- **Always refused:** `stratum_merge_change`, `stratum_reject_change`, and
  ...
  [change comments](/guides/code-review/), which the REST API accepts from agents
  but this server does not yet expose as a tool.

So an agent-token deployment is a *proposer*: it forks, commits, opens the
change, and a human (over the UI, [CLI](/guides/cli/), or their own MCP session with
...
- Evaluation runs inside `stratum_create_change`, so on a project with a slow
  ...
  [evaluation budget](/guides/troubleshooting/#sandbox-budget-exceeded-install-or-command)
  allows.

## Reference

- [`mcp/README.md`](https://github.com/stratum-eng/stratum/blob/main/mcp/README.md) — the package's own quick reference
